...
def open_image(PATH, basewidth=300):
    '''이거 변수 이름 바꾼거임.... basewidth는 사실 baseheight라는 뜻이죠...'''
...
def next_page():
    ...
    IMG_NAME = update_df.loc[idx, '이미지파일명']
...
try:
    ...
    update_df = df.copy()
    update_df[['확인여부', '삭제여부', '광고여부']] = [False, False, False]
    joblib.dump(update_df, UPDATE_DATAFRAME_PATH)
# 원본파일에 수정용 파일을 반영해서 데이터프레임 생성 해야함(index를 초기화 하지 않고하면 되지 않을까?).
################################################
...
#######초기 설정 ################################
idx = 0  # 초깃값: 0
total_page = len(update_df)  # 여기에 len(df)가 들어감
...
current_page_num = tkinter.Label(window, text=f"{idx + 1} / {total_page}")
...
current_page_num.place(x=50, y=20)
